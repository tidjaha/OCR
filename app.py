...
# -----------------------------
st.title(
    "Bienvenue dans mon projet OCR ! Ce travail s’inscrit dans le cadre de ma formation "
    "en deep learning, qui est encore en cours et devrait se terminer dans environ un mois."
)
...
def reset_app():
    st.session_state.uploaded_file = None
    st.session_state.image_confirmee = False
    st.session_state.uploaded_file_key += 1  # Change la key pour reconstruire le uploader
    st.experimental_rerun()  # relance le script pour reconstruire le site

# -----------------------------
# File uploader avec key dynamique
# -----------------------------
uploaded_file = st.file_uploader(
    "Choisis une image à analyser (PNG, JPG, JPEG)",
    type=["png", "jpg", "jpeg"],
...
if st.session_state.uploaded_file is not None:
    ...
    with col1:
        if st.button("Oui"):
            ...
            st.success("Image confirmée ! On continue le traitement…")
            ...
            if not boxes:
                ...
                for idx, (x1, y1, x2, y2, conf) in enumerate(boxes_sorted, start=1):
                    ...
                    st.image(crop_pil, caption=f"Aperçu crop {idx}", use_container_width=True)
    ...
    with col2:
        if st.button("Non"):
            reset_app()  # ici le site revient à zéro et le uploader se reconstruit
